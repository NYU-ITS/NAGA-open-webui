"""
title: SMART - Portkey Edition
author: ps5226 (modified)
description: SMART is a sequential multi-agent reasoning technique (Portkey-only version).
required_open_webui_version or greater: 0.5.0
requirements: langchain-openai==0.2.14, langgraph==0.2.60, portkey-ai
version: 1.1
licence: MIT
"""
...
class Pipe:
    class Valves(BaseModel):
        ...
        PORTKEY_API_KEY: str = Field(
            default="NvjcqtGRqO4jxq7lrz3YVgludhco",
            description="API key for Portkey",
        )
        PORTKEY_VIRTUAL_KEYS: dict = Field(
            default={
                "anthropic": "anthropic-virtu-2849df",
                "google": "gemini-a67b98",
                "openai": "openai-nyu-it-d-5b382a",
                "hpc": "portkey-on-open-5d7656",
            },
            description="Virtual keys for each provider",
        )
    ...
    def extract_model_name(self, model_id):
        # Extract everything after the last '/'
        match = re.search(r"\/([^\/]+)$", model_id)
        if match:
            return match.group(1)
        return model_id  # Return original if no '/' found

    def setup(self):
        v = self.valves
        if not v.PORTKEY_API_KEY:
            raise Exception("Error: PORTKEY_API_KEY is not set")

        # Extract provider and model from model name string
        def extract_provider_from_model_id(model_id):
            provider = model_id.split("/")[0].lower()
            return provider

        # Setup Portkey headers creation function
        def get_portkey_headers(model_id):
            provider = extract_provider_from_model_id(model_id)
            if provider not in v.PORTKEY_VIRTUAL_KEYS:
                raise ValueError(f"No virtual key configured for provider: {provider}")

            return createHeaders(
                api_key=v.PORTKEY_API_KEY, virtual_key=v.PORTKEY_VIRTUAL_KEYS[provider]
            )

        self.get_portkey_headers = get_portkey_headers
    ...
    async def pipe(
        ...
        try:
            ...
            import types
            import langchain_openai.chat_models.base as lcb

            def patched_create_usage_metadata(oai_token_usage):
                input_tokens = oai_token_usage.get("prompt_tokens", 0) or 0
                output_tokens = oai_token_usage.get("completion_tokens", 0) or 0
            ...
            lcb._create_usage_metadata = patched_create_usage_metadata
            ...
            # Create planning model with Portkey
            planning_provider = planning_model_id.split("/")[0].lower()
            planning_model = ChatOpenAI(
                model=self.extract_model_name(planning_model_id),
                api_key="PORTKEY_MANAGED",
                base_url=self.valves.PORTKEY_API_BASE_URL,
                default_headers=self.get_portkey_headers(planning_model_id),
            )

            planning_buffer = ""
            async for chunk in planning_model.astream(planning_messages):
            ...
            # Create the model using Portkey
            model_to_use = ChatOpenAI(
                model=self.extract_model_name(model_to_use_id),
                api_key="PORTKEY_MANAGED",
                base_url=self.valves.PORTKEY_API_BASE_URL,
                default_headers=self.get_portkey_headers(model_to_use_id),
            )
            ...
            if is_reasoning_needed == "NO":
                ...
                # Create reasoning model with Portkey
                reasoning_model = ChatOpenAI(
                    model=self.extract_model_name(reasoning_model_id),
                    api_key="PORTKEY_MANAGED",
                    base_url=self.valves.PORTKEY_API_BASE_URL,
                    default_headers=self.get_portkey_headers(reasoning_model_id),
                )
                ...
                reasoning_bufffer = ""
                update_status = 0
                async for chunk in reasoning_model.astream(reasoning_messages):
                    content = chunk.content
                    assert isinstance(content, str)
                ...
                if len(tool_agent_content) > 0:
                    ...
                    if not __tools__:
                        ...
                        # Create large model with Portkey for tool agent
                        large_model = ChatOpenAI(
                            model=large_model_id,
                            api_key="PORTKEY_MANAGED",
                            base_url=self.valves.PORTKEY_API_BASE_URL,
                            default_headers=self.get_portkey_headers(large_model_id),
                        )
                        ...
                        message_buffer = ""
                        num_tool_calls = 0
                        async for event in graph.astream_events(inputs, version="v2"):  # type: ignore
                            if num_tool_calls > 3:
                                yield "[TOO MANY TOOL CALLS - AGENT TERMINATED]"
                                break
                            kind = event["event"]
                ...
                async for event in graph.astream_events(inputs, version="v2"):  # type: ignore
                    if num_tool_calls >= 6:
                        await send_status(
                            status_message="Interrupting due to max tool calls reached!",
                            done=True,
                        )
                        yield "[TOO MANY TOOL CALLS - AGENT TERMINATED]"
                        break
