...
from open_webui.models.models import Model, ModelForm, Models, ModelModel, ModelMeta
...
def reset_vector_db(user=Depends(get_admin_user)):
    """
    Complete reset of vector database, SQL tables, physical files, and model references.
    This is a DESTRUCTIVE operation that removes ALL vector data, knowledge bases, files,
    memories, and cleans up related references.
    
    WARNING: This action cannot be undone. Use with caution.
    NOTE: This is ONLY triggered manually via the API endpoint, NOT on pod restart or initialization.
    """
    errors = []
    warnings = []
    
    try:
        # STEP 1: Vector DB Reset (Non-destructive - can verify)
        log.info("Starting vector database reset...")
        try:
            VECTOR_DB_CLIENT.reset()
            log.info("Vector database reset completed successfully")
        except Exception as e:
            error_msg = f"Error resetting vector database: {e}"
            log.exception(error_msg)
            errors.append(error_msg)
            # Continue - SQL cleanup can still proceed
        
        # STEP 2: SQL Table Cleanup (in order of dependencies)
        log.info("Cleaning SQL tables...")
        
        # 2.1: Delete all knowledge bases
        try:
            result = Knowledges.delete_all_knowledge()
            if result:
                log.info("All knowledge bases deleted from SQL")
            else:
                warnings.append("Failed to delete knowledge bases from SQL")
        except Exception as e:
            error_msg = f"Error deleting knowledge bases: {e}"
            log.exception(error_msg)
            errors.append(error_msg)
        ...
        try:
            ...
            if result:
                log.info("All files deleted from SQL")
            else:
                warnings.append("Failed to delete files from SQL")
        except Exception as e:
            error_msg = f"Error deleting files: {e}"
            log.exception(error_msg)
            errors.append(error_msg)
        
        # 2.3: Delete all memories
        try:
            result = Memories.delete_all_memories()
            if result:
                log.info("All memories deleted from SQL")
            else:
                warnings.append("Failed to delete memories from SQL")
        except Exception as e:
            error_msg = f"Error deleting memories: {e}"
            log.exception(error_msg)
            errors.append(error_msg)
        
        # STEP 3: Physical File Deletion
        log.info("Deleting physical files from storage...")
        try:
            Storage.delete_all_files()
            log.info("All physical files deleted from storage")
        except Exception as e:
            error_msg = f"Error deleting physical files: {e}"
            log.exception(error_msg)
            errors.append(error_msg)
            # Non-critical - files may have been deleted already
        
        # STEP 4: Model Reference Cleanup
        log.info("Cleaning up model references to knowledge bases...")
        try:
            with get_db() as db:
                ...
                for model_row in all_models:
                    try:
                        model = ModelModel.model_validate(model_row)
                        
                        # Check if model has knowledge references
                        # Handle both dict and object access (JSONField can return dict or ModelMeta)
                        knowledge_list = None
                        if model.meta:
                            if isinstance(model.meta, dict):
                                knowledge_list = model.meta.get("knowledge", [])
                            elif hasattr(model.meta, "knowledge"):
                                knowledge_list = getattr(model.meta, "knowledge", []) or []
                        
                        if knowledge_list:
                            log.info(f"Clearing knowledge references from model {model.id}")
                            
                            # Update meta - ModelMeta allows extra fields via ConfigDict(extra="allow")
                            if isinstance(model.meta, dict):
                                # If it's a dict, update it directly
                                model.meta["knowledge"] = []
                                # Convert to ModelMeta for ModelForm (Pydantic will validate)
                                meta_obj = ModelMeta.model_validate(model.meta)
                            else:
                                # If it's already a ModelMeta object, set the field directly
                                setattr(model.meta, "knowledge", [])
                                meta_obj = model.meta
                        ...
                        log.warning(f"Error updating model {model_row.id}: {e}")
                        warnings.append(f"Failed to update model {model_row.id}")
                
                log.info(f"Updated {updated_count} models to remove knowledge references")
        except Exception as e:
            error_msg = f"Error cleaning model references: {e}"
            log.exception(error_msg)
            errors.append(error_msg)
        
        # STEP 5: Response
        if errors:
            log.error(f"Reset completed with errors: {errors}")
            if warnings:
                log.warning(f"Warnings: {warnings}")
            return {
                "status": "completed_with_errors",
                "message": "Reset completed but some errors occurred",
                "errors": errors,
                "warnings": warnings
            }
        elif warnings:
            log.warning(f"Reset completed with warnings: {warnings}")
            return {
                "status": "completed_with_warnings",
                "message": "Reset completed successfully with some warnings",
                "warnings": warnings
            }
        else:
            log.info("Vector database reset completed successfully")
            return {
                "status": "success",
                "message": "Vector database reset completed successfully"
            }
            
    except Exception as e:
        log.exception(f"Fatal error during reset: {e}")
        return {
            "status": "failed",
