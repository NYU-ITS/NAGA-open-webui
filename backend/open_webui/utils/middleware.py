            ...
            with ThreadPoolExecutor() as executor:
                sources = await loop.run_in_executor(
                    ...
                    lambda: get_sources_from_files(
                        ...
                        hybrid_search=request.app.state.config.ENABLE_RAG_HYBRID_SEARCH.get(
                            user.email
                        ),
                        full_context=request.app.state.config.RAG_FULL_CONTEXT.get(
                            user.email
                        ),
                    ),
                )
    ...
    if len(sources) > 0:
        ...
        if model.get("owned_by") == "ollama":
            form_data["messages"] = prepend_to_first_user_message_content(
                rag_template(
                    request.app.state.config.RAG_TEMPLATE.get(user.email),
                    context_string,
                    prompt,
                ),
                form_data["messages"],
