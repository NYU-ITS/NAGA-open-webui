...
async def chat_completed(request: Request, form_data: dict, user: Any):
    ...
    if getattr(request.state, "direct", False) and hasattr(request.state, "model"):
        ...
        log.debug("[DEBUG] [inside chat_completed() from chat.py] Using get_models_for_request.")
        models = get_models_for_request(request)
    log.debug(f"[DEBUG] [inside chat_completed() from chat.py] ensure_models_for_request completed. models has {len(models)} entries.")
