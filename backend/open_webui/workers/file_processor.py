...
def get_worker_config():
    ...
    if _worker_config is None:
        try:
            ...
            _worker_config.RAG_EMBEDDING_MODEL_USER = (
                RAG_EMBEDDING_MODEL_USER  # RBAC: Per-admin model name
            )
            ...
            # Additional UserScopedConfig objects used in worker code paths
            _worker_config.RAG_TEMPLATE = (
                RAG_TEMPLATE  # Used in _get_user_chunk_settings
            )
            _worker_config.TOP_K = (
                RAG_TOP_K  # Used in retrieval queries (aliased as TOP_K in main.py)
            )
            _worker_config.RAG_TOP_K = RAG_TOP_K  # Also set alias for compatibility
            _worker_config.ENABLE_RAG_HYBRID_SEARCH = (
                ENABLE_RAG_HYBRID_SEARCH  # Used in retrieval queries
            )
            _worker_config.RAG_FULL_CONTEXT = (
                RAG_FULL_CONTEXT  # Used in config endpoints
            )
            _worker_config.RELEVANCE_THRESHOLD = (
                RAG_RELEVANCE_THRESHOLD  # PersistentConfig, used in retrieval
            )
            ...
            _worker_config.PDF_IMAGE_DESCRIPTION_MODEL_USER = (
                PDF_IMAGE_DESCRIPTION_MODEL_USER
            )
            _worker_config.DOCUMENT_INTELLIGENCE_ENDPOINT = (
                DOCUMENT_INTELLIGENCE_ENDPOINT
            )
            _worker_config.DOCUMENT_INTELLIGENCE_KEY = DOCUMENT_INTELLIGENCE_KEY
            _worker_config.BYPASS_EMBEDDING_AND_RETRIEVAL = (
                BYPASS_EMBEDDING_AND_RETRIEVAL
            )
            ...
            _worker_config.ENABLE_EVALUATION_ARENA_MODELS = (
                ENABLE_EVALUATION_ARENA_MODELS
            )
            ...
            if (
                not hasattr(_worker_config, "_state")
                or "RAG_EMBEDDING_ENGINE" not in _worker_config._state
            ):
                raise ValueError(
                    "AppConfig._state not properly populated with RAG_EMBEDDING_ENGINE"
                )

            log.info(
                "Worker AppConfig initialized successfully with all RAG config values (cached for reuse)"
            )
...
class MockState:
    ...
    def initialize_embedding_function(
        ...
        try:
            ...
            model_to_use = (
                embedding_model if embedding_model else self.config.RAG_EMBEDDING_MODEL
            )
            ...
            log.info(
                f"[RBAC] Using embedding model from job: {model_to_use} (RBAC-protected)"
            )
            ...
            error_msg = (
                f"Failed to initialize EMBEDDING_FUNCTION per-job: {embedding_error}"
            )
...
class _FallbackConfig:
    ...
    def __init__(self):
        ...
        self.RAG_EMBEDDING_MODEL = os.environ.get(
            "RAG_EMBEDDING_MODEL", "@openai-embedding/text-embedding-3-small"
        )
        ...
        self.RAG_EMBEDDING_MODEL_USER = MockUserScopedConfig(
            os.environ.get(
                "RAG_EMBEDDING_MODEL", "@openai-embedding/text-embedding-3-small"
            )
        )
        self.RAG_EMBEDDING_BATCH_SIZE = int(
            os.environ.get("RAG_EMBEDDING_BATCH_SIZE", "1")
        )
        ...
        self.RAG_OPENAI_API_KEY = MockUserScopedConfig(
            os.environ.get("RAG_OPENAI_API_KEY", "")
        )
        self.RAG_OLLAMA_BASE_URL = os.environ.get("RAG_OLLAMA_BASE_URL", "")
        self.RAG_OLLAMA_API_KEY = os.environ.get("RAG_OLLAMA_API_KEY", "")
        self.CONTENT_EXTRACTION_ENGINE = os.environ.get(
            "CONTENT_EXTRACTION_ENGINE", "auto"
        )
        ...
        self.CHUNK_SIZE = MockUserScopedConfig(
            int(os.environ.get("CHUNK_SIZE", "1000"))
        )
        self.CHUNK_OVERLAP = MockUserScopedConfig(
        ...
        self.RAG_TEMPLATE = MockUserScopedConfig(
        ...
        self.ENABLE_RAG_HYBRID_SEARCH = MockUserScopedConfig(
            os.environ.get("ENABLE_RAG_HYBRID_SEARCH", "False").lower() == "true"
        )
        # RAG_FULL_CONTEXT is used in config endpoints
        self.RAG_FULL_CONTEXT = MockUserScopedConfig(
            os.environ.get("RAG_FULL_CONTEXT", "False").lower() == "true"
        )
        ...
        self.BYPASS_EMBEDDING_AND_RETRIEVAL = (
...
def process_file_job(
    ...
    log.info(
        f"[JOB] START | file_id={file_id} | user_id={user_id} | knowledge_id={knowledge_id} | timestamp={start_time:.3f}"
    )
    ...
    try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        error_msg = (
                            f"Failed to initialize EMBEDDING_FUNCTION: {init_error}"
                        )
                        log.error(error_msg, exc_info=True)
                        raise
                except Exception as init_error:
                    log.error(
                        f"Failed to initialize MockRequest for file_id={file_id}: {init_error}",
                        exc_info=True,
                    )
                    raise
                ...
                try:
                                    ...
                                    try:
                                        ...
                                        if (
                                            ...
                                            request.app.state.config.RAG_EMBEDDING_MODEL_USER.set(
                                                owner_email, embedding_model
                                            )
                                            request.app.state.config.RAG_OPENAI_API_KEY.set(
                                                owner_email, embedding_api_key
                                            )
                                            log.info(
                                        ...
                                        log.error(
                                            f"Could not update config with owner model/key: {config_update_error}"
                                        )
                                        raise
                    ...
                    if not file:
                        ...
                        log.error(
                            f"File {file_id} not found for processing (user_id={user_id})"
                        )
                    ...
                    vector_collection_name = (
                        collection_name if collection_name else f"file-{file.id}"
                    )
                    ...
                    if content:
                        ...
                        log.info(
                            f"[JOB] KNOWLEDGE_COLLECTION | file_id={file.id} | collection={collection_name} | checking if already processed"
                        )
                        ...
                        if (
                            ...
                            log.info(
                                f"[JOB] ALREADY_PROCESSED | file_id={file.id} | found {len(result.ids[0])} existing documents"
                            )
                            # File already processed - extract from vector DB
                            try:
                                # Safely access result structure
                                if (
                                    hasattr(result, "documents")
                                    and result.documents
                                    and hasattr(result, "metadatas")
                                    ...
                                    log.warning(
                                        f"Invalid result structure for file_id={file.id}, falling back to extraction"
                                    )
                                    docs = None
                                    text_content = None
                            except (
                                ...
                                log.warning(
                                    f"Error accessing result structure for file_id={file.id}: {result_error}, falling back to extraction",
                                    exc_info=True,
                                )
                                docs = None
                                text_content = None
                            ...
                            if docs is None:
                                file_content = (
                                    file.data.get("content", "") if file.data else ""
                                )
                                    ...
                                    if file_path:
                                        try:
                                            ...
                                            if file_path and os.path.exists(file_path):
                                                ...
                                                log.info(
                                                    f"[FILE] id={file.id} | name={file.filename} | size={file_size}B | path={file_path}"
                                                )
                                                ...
                                                try:
                                                    print(
                                                        f"[DEBUG] About to call loader.load() for file_id={file.id} | extract_images={pdf_extract_images_val}",
                                                        flush=True,
                                                    )
                                                    log.info(
                                                        f"[DEBUG] About to call loader.load() for file_id={file.id} | extract_images={pdf_extract_images_val}"
                                                    ...
                                                    print(
                                                        f"[DEBUG] loader.load() completed for file_id={file.id} | docs_count={len(docs) if docs else 0}",
                                                        flush=True,
                                                    )
                                                    log.info(
                                                        f"[DEBUG] loader.load() completed for file_id={file.id} | docs_count={len(docs) if docs else 0}"
                                                    ...
                                                    log.info(
                                                        f"[EXTRACT] SUCCESS | file_id={file.id} | docs={len(docs) if docs else 0} | chars={total_chars} | non_empty={non_empty}"
                                                    )
                                                except Exception as load_error:
                                                    log.error(
                                                        f"[EXTRACT] FAILED | file_id={file.id} | error={type(load_error).__name__}: {load_error}",
                                                        exc_info=True,
                                                    )
                                                    docs = []
                                                ...
                                                log.warning(
                                                    f"File path is None or does not exist, using empty content"
                                                )
                                                docs = [
                                                    Document(
                                                        page_content="",
                                            ...
                                            print(
                                                f"    ❌ Failed to get file from storage: {storage_error}",
                                                flush=True,
                                            )
                                            log.error(
                                                f"    ❌ Failed to get file from storage: {storage_error}",
                                        ...
                                        log.error(
                                            f"[JOB] FILE_PATH_MISSING | file_id={file.id} | filename={file.filename} | file.path is None - cannot extract content"
                                        )
                        ...
                        log.info(
                            f"[JOB] REGULAR_UPLOAD | file_id={file.id} | filename={file.filename} | file.path={file.path}"
                        )
                        ...
                        if not file_content or len(file_content.strip()) == 0:
                            ...
                            if file_path:
                                log.info(
                                    f"[JOB] FILE_PATH_EXISTS | file_id={file.id} | path={file_path}"
                                )
                                try:
                                    file_path = Storage.get_file(file_path)
                                    if file_path and os.path.exists(file_path):
                                        ...
                                        try:
                                            print(
                                                f"[DEBUG] About to call loader.load() for file_id={file.id} | extract_images={pdf_extract_images_val} | timestamp={extract_start:.3f}",
                                                flush=True,
                                            )
                                            log.info(
                                                f"[DEBUG] About to call loader.load() for file_id={file.id} | extract_images={pdf_extract_images_val}"
                                            ...
                                            print(
                                                f"[DEBUG] loader.load() completed for file_id={file.id} | docs_count={len(docs) if docs else 0} | duration={extract_duration:.2f}s",
                                                flush=True,
                                            )
                                            log.info(
                                                f"[DEBUG] loader.load() completed for file_id={file.id} | docs_count={len(docs) if docs else 0} | duration={extract_duration:.2f}s"
                                            ...
                                            log.info(
                                                f"[EXTRACT] SUCCESS | file_id={file.id} | docs={len(docs) if docs else 0} | chars={total_chars} | non_empty={non_empty} | duration={extract_duration:.2f}s | timestamp={extract_end:.3f}"
                                            )
                                            log.debug(
                                                f"EXTRACT SUCCESS | docs={len(docs) if docs else 0} | chars={total_chars}"
                                            )
                                            safe_add_span_event(
                                                "job.file.extracted",
                                                {
                                            ...
                                            log.debug(
                                                f"After metadata addition | docs={len(docs)} | text_content_len={len(text_content)}"
                                            )
                                        except Exception as load_error:
                                            log.error(
                                                f"[EXTRACT] FAILED | file_id={file.id} | error={type(load_error).__name__}: {load_error}",
                                            ...
                                            log.debug(
                                                f"EXTRACT FAILED | error={load_error}"
                                            )
                                            docs = []
                                    else:
                                        log.error(
                                            f"[JOB] FILE_NOT_FOUND | file_id={file.id} | resolved_path={file_path} | file does not exist"
                                        )
                                        error_msg = (
                                    ...
                                    log.error(
                                        f"[JOB] STORAGE_ERROR | file_id={file.id} | error={type(storage_error).__name__}: {storage_error}",
                                        exc_info=True,
                                    )
                            ...
                            # Use content from file.data
                            log.info(
                                f"[JOB] USING_FILE_DATA | file_id={file.id} | content_length={len(file_content)}"
                            )
                            log.debug(
                                f"USING_FILE_DATA branch | file_content_len={len(file_content)}"
                            )
                            docs = [
                                Document(
                                    page_content=file_content,
                            ...
                            text_content = file_content
                            log.debug(
                                f"After USING_FILE_DATA | docs={len(docs)} | text_content_len={len(text_content)}"
                            )

                    # Log state after if/else block with timestamp
                    current_time = time.time()
                    log.debug(
                        f"After REGULAR_UPLOAD if/else block | timestamp={current_time:.3f} | elapsed={current_time - start_time:.2f}s"
                    )
                    log.debug(f"  'docs' in locals: {'docs' in locals()}")
                    log.debug(
                        f"  docs count: {len(docs) if 'docs' in locals() and docs else 'UNDEFINED or None'}"
                    )
                    log.debug(
                        f"  'text_content' in locals: {'text_content' in locals()}"
                    )
                    ...
                    error_msg = str(e)
                    log.error(
                        f"[JOB] FAILED | file_id={file_id} | user_id={user_id} | elapsed={elapsed_time:.2f}s | timestamp={error_time:.3f} | error={type(e).__name__}: {error_msg}",
                        exc_info=True,
                    )
                ...
                try:
                    ...
                    if docs and len(docs) > 0:
                        log.debug(
                            f"  first doc page_content length: {len(docs[0].page_content) if docs[0].page_content else 0}"
                        )
                ...
                log.info(
                    f"[VALIDATE] START | file_id={file.id} | docs={len(docs) if docs else 0} | chars={total_chars} | non_empty={non_empty} | timestamp={validate_start:.3f} | elapsed={validate_start - start_time:.2f}s"
                )
                log.debug(
                    f"VALIDATION | docs={len(docs) if docs else 0} | chars={total_chars} | non_empty={non_empty}"
                )

                if not docs or len(docs) == 0:
                    error_msg = ERROR_MESSAGES.EMPTY_CONTENT
                    log.error(
                        f"[VALIDATE] FAILED | file_id={file.id} | reason=NO_DOCS | loader returned empty list"
                    )
                    Files.update_file_metadata_by_id(
                        file.id,
                        {"processing_status": "error", "processing_error": error_msg},
                    )
                    raise ValueError(error_msg)
                ...
                if total_chars == 0:
                    error_msg = ERROR_MESSAGES.EMPTY_CONTENT
                    log.error(
                        f"[VALIDATE] FAILED | file_id={file.id} | reason=EMPTY_CONTENT | docs={len(docs)} but all page_content empty"
                    )
                    log.error(
                        f"[VALIDATE] DIAGNOSIS | Possible: image-only PDF, corrupted file, extraction failed silently"
                    )
                    Files.update_file_metadata_by_id(
                        file.id,
                        {"processing_status": "error", "processing_error": error_msg},
                ...
                log.info(
                    f"[VALIDATE] PASSED | file_id={file.id} | docs={len(docs)} | chars={total_chars} | non_empty={non_empty} | duration={validate_duration:.2f}s | timestamp={validate_end:.3f} | elapsed={validate_end - start_time:.2f}s"
                )
                ...
                if not bypass_embedding:
                    ...
                    log.info(
                        f"[RAG File] file_id={file.id} | filename={filename} | docs_pre_split={len(docs)} (chunking and embedding next)"
                    )
                    log.info(
                        f"[EMBED] START | file_id={file.id} | filename={filename} | docs_pre_split={len(docs)} | timestamp={embed_start:.3f}"
                    )
                    try:
                        ...
                        if knowledge_id:
                            ...
                            if result:
                                ...
                                log.info(
                                    f"[EMBED] SUCCESS | file_id={file.id} | filename={filename} | collections={collections} | docs_pre_split={len(docs)} | duration={embed_duration:.2f}s | timestamp={embed_end:.3f}"
                                )
                                safe_add_span_event(
                                ...
                                log.error(
                                    f"[EMBED] FAILED | file_id={file.id} | filename={filename} | reason=SAVE_TO_VDB_FAILED"
                                )
                                Files.update_file_metadata_by_id(
                                    file.id,
                                    {
                            ...
                            file_collection = f"file-{file.id}"
                            log.info(
                                f"[EMBED] SINGLE_COLLECTION | file_id={file.id} | filename={filename} | collection={file_collection} | docs_pre_split={len(docs)} | timestamp={time.time():.3f}"
                            )
                            ...
                            if result:
                                ...
                                embed_duration = embed_end - embed_start
                                log.info(
                                    f"[EMBED] SUCCESS | file_id={file.id} | filename={filename} | collection={collection_name} | docs_pre_split={len(docs)} | duration={embed_duration:.2f}s | timestamp={embed_end:.3f}"
                                )
                                Files.update_file_metadata_by_id(
                                    file.id,
                                    {
                        ...
                        error_msg = str(e)
                        log.error(
                            f"[EMBED] FAILED | file_id={file.id} | filename={filename} | error={type(e).__name__}: {error_msg}",
                            exc_info=True,
                        )
                        try:
                            Files.update_file_metadata_by_id(
                            ...
                            log.error(
                                f"Failed to update file status after vector DB error: {update_error}"
                            )
                        return {"status": "error", "error": error_msg}
                else:
                    Files.update_file_metadata_by_id(
                ...
                safe_set_span_attribute(
                    span, "job.duration_ms", int(elapsed_time * 1000)
                )
        ...
        error_msg = str(e)
        log.error(
            f"[JOB] FAILED | file_id={file_id} | user_id={user_id} | elapsed={elapsed_time:.2f}s | error={type(e).__name__}: {error_msg}",
            exc_info=True,
        )
        ...
        try:
            ...
            log.warning(
                f"Error cleaning up database session registry: {session_cleanup_error}"
            )
        ...
        try:
            ...
            # Log but don't fail - cleanup errors shouldn't break the job result
            log.warning(
                f"Error cleaning up embedding function: {embedding_cleanup_error}"
            )
